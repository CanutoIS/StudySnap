...
describe('retrievePosts', () => {
    ...
    it('succeeds on rtrieving all posts', async () => {
        try {
            ...
            const postsFound = await retrievePosts(userId, 'Show all posts')
    ...
    it('succeeds on rtrieving posts from a specific subject', async () => {
        try {
            const _user = await User.findOne({ email: user.email })
            const userId = _user._id.toString()
            ...
            await Post.create({ author: new ObjectId(userId), title: postTitle2, text: postText2, subject: 'Chemistry' })

            const postsFound = await retrievePosts(userId, 'History')
    ...
    it('fails on non-existing user', async () => {
        try {
            ...
            await retrievePosts(wrongUserId, 'Others')
