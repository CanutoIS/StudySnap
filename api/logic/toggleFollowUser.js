  ...
  return (async () => {
    ...
    let followingUsers = user.following.map(follow => follow.id.toString())
      ...
      if(followingUsers.includes(profileUserId)) {
        ...
        await User.updateOne(
          { _id: userId },
          { $push: { following: { id: new ObjectId(profileUserId), username: profileUser.username, name: profileUser.name } } }
        )
        await User.updateOne(
          { _id: profileUserId },
          { $push: { followers: { id: new ObjectId(userId), name: user.name, username: user.username } } }
        )
      }
  })()
}