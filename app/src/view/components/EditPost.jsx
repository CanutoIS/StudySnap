...
export default function EditPost({ onUpdatedPost, onCancel }) {
    ...
    return <ModalContainer className='EditPostContainer fixed bg-black bg-opacity-20 top-0 left-0 z-30' onClick={event => {
        ...
        {post && <ModalWindow className='w-11/12 h-5/6 md:w-2/3'>
            <h2 className="text-2xl">Edit post</h2>
            <Form className='flex flex-col items-center gap-2 w-full h-5/6 md:text-lg' onSubmit={handleUpdatePost}>
                    <Input className='w-full' name='title' placeholder="Summary title" defaultValue={post.title}></Input>

                    <textarea className="w-full h-2/3 rounded-lg border border-gray-400 p-2" name="content" placeholder="Summary content" cols="30" rows="10" defaultValue={post.text}></textarea>
