...
export default function ContextualMenu({ options, toggleContextualMenu, classNameContainer, classNameMenu }) {
    return <ModalContainer className={`ContainerContextualMenu absolute top-0 left-0 ${classNameContainer ? classNameContainer : 'file:'}`} onClick={event => {
        ...
        <section className={`w-40 bg-white border border-black rounded-xl flex flex-col items-center absolute right-8 ${classNameMenu ? classNameMenu : 'top-24 md:top-28'} md:right-72 md:text-lg md:w-44`}>
            {options.map((option, index) => {
                const isLast = index === options.length - 1

                return <div
                    className={`flex justify-center items-center h-10 w-full ${isLast ? '' : 'border-b border-black'} cursor-pointer`}
